...
class NgramHashMapping:
    ...
    def _get_ngram_hashes(
        ...
        head_vocab_sizes = self.vocab_size_across_layers[layer_id]

        return flash_engram._get_ngram_hashes(x, B, T, multipliers, np.asarray(head_vocab_sizes, dtype=np.int64),
            self.max_ngram_size-1, self.n_head_per_ngram, self.pad_id)
...
class MultiHeadEmbedding(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        output = flash_engram._lookup_f32(self.embedding.num_embeddings, self.embedding.embedding_dim, self.embedding.weight,
            input_ids, B, T, H, self.offsets, 1)
        return output
